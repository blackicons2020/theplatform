...
/**
 * The People's Platform - Backend Server Code (PostgreSQL Version)
 * 
 * NOTE: This file is for reference only. It cannot run in the browser environment.
 * 
 * Setup Instructions:
 * 1. Create a new folder on your computer.
 * 2. Run `npm init -y`
 * 3. Run `npm install express pg cors dotenv`
 * 4. Create a .env file with: PORT=5000 and DATABASE_URL=postgresql://user:password@localhost:5432/peoples_platform
 * 5. Paste this code into `server.js`
...
const pool = new Pool({
  connectionString: process.env.DATABASE_URL || 'postgresql://postgres:password@localhost:5432/peoples_platform',
  // ssl: { rejectUnauthorized: false } // Uncomment if deploying to Heroku/Render/Neon
});
...
app.get('/api/ads/active', async (req, res) => {
  try {
    ...
    // Map DB columns (snake_case) to JSON keys expected by frontend (camelCase)
